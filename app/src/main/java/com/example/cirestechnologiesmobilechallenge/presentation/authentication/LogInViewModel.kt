...
class LogInViewModel(): ViewModel() {
    ...
    fun signIn(user: User) {
        viewModelScope.launch {
            when(user) {
                Constants.USER_VALID  -> {
                    _eventFlow.emit(UIEvent.LogInButtonPressed(Constants.SUCCESS));
                }
                Constants.USER_BLOCKED  -> {
                    _eventFlow.emit(UIEvent.LogInButtonPressed(Constants.BLOCKED))
                }
